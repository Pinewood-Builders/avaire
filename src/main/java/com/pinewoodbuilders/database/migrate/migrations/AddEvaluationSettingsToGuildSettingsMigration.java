...
public class AddEvaluationSettingsToGuildSettingsMigration implements Migration {

    @Override
    public String created_at() {
        return "Mon, Jun 6, 2022 2:39 PM";
    }

    @Override
    public boolean up(Schema schema) throws SQLException {
        if (schema.hasColumn(Constants.GUILD_SETTINGS_TABLE, "evaluation_settings")) {
            return true;
        }
    ...
    public boolean down(Schema schema) throws SQLException {
        ...
        schema.getDbm().queryUpdate(String.format(
            "ALTER TABLE `%s` DROP `evaluation_settings`;",
            Constants.GUILD_SETTINGS_TABLE
        ));
